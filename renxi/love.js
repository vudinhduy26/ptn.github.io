(function(window){
    ...
    Person.prototype = {
        ...
        draw: function() {
            ...
            // Vẽ tay cong đẹp hơn (màu đen)
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 6;
            ctx.beginPath();
            // Tay trái
            ctx.moveTo(0, -20);
            ctx.quadraticCurveTo(-25, -35, -20, -60);
            // Tay phải  
            ctx.moveTo(0, -20);
            ctx.quadraticCurveTo(25, -35, 20, -60);
            ctx.stroke();
            
            // Vẽ chân cong đẹp hơn (màu đen)
            ctx.lineWidth = 6;
            ctx.beginPath();
            // Chân trái
            ctx.moveTo(0, 15);
            ctx.quadraticCurveTo(-10, 25, -12, 45);
            // Chân phải
            ctx.moveTo(0, 15);
            ctx.quadraticCurveTo(10, 25, 12, 45);
            ctx.stroke();
            
            // Vẽ trái tim ở trên đầu (giữa hai tay)
            var heart = new Heart();
            ctx.fillStyle = '#FF1744';
            ctx.beginPath();
            ctx.moveTo(0, -70);
            for (var i = 0; i < heart.length; i++) {
                var p = heart.get(i, 1.0);
